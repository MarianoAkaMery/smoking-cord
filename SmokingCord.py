...
#------------------------------------------------------#
client = MongoClient('mongodb+srv://urlConnectionString')
...
async def count_function(ctx: interactions.ComponentContext):
    ...
    if (UserCollection.count_documents(query) == 0):
        user = {"UserId": f"{userid}", "Name": f"{ctx.author.name}", "ServerId": f"{ctx.guild.rules_channel_id._snowflake}","LastUse": f"{datagiorno}/{datamese}/{dataanno}" ,"DailyCount": 0, "TotalCount": 0}
